...
class IDCardGenerator:
    def __init__(self, template_path, photos_folder, qr_folder, excel_path, output_folder, coordinates=None, photo_size=(240, 240), qr_size=(100, 100), log_callback=None, export_as_pdf_var=None):
        """
    ...
    def generate_all_id_cards(self):
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    if generated_card_image:
                        if self.export_as_pdf_var.get(): # Check if PDF export is selected using the stored variable
                            generated_images.append(generated_card_image)
                            self.log_callback(f"  ✅ Generated image for {ext_id} (added to PDF list)")
                        else:
                            # Save individual PNG if not exporting as PDF
                            output_path = os.path.join(
                                self.output_folder,
                                f"{ext_id}_id.png"
                            )
                            generated_card_image.save(output_path)
                            self.log_callback(f"  ✅ ID card saved: {os.path.basename(output_path)}")
            ...
            # Save as PDF if selected
            if self.export_as_pdf_var.get() and generated_images:
            ...
            elif self.export_as_pdf_var.get() and not generated_images:
                self.log_callback("⚠️ No cards generated to save as PDF.")
            else:
                self.log_callback(f"  • Output folder: {self.output_folder} (individual PNGs)")
